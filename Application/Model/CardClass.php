...
class CardClass extends HS_DatabaseObject {
	protected $id; // Int
	...
	public static function fromDB($id, $subdomain = null) {
		global $OBJECT_MAP;
		if ( ! isset($OBJECT_MAP["CardClass"])) {
			$OBJECT_MAP["CardClass"] = array();
		}
		$obj = null;
		if (isset($OBJECT_MAP["CardClass"][$id])) {
			$obj = $OBJECT_MAP["CardClass"][$id];
		} else {
			$obj = parent::fromDBHelper(new self($subdomain), array("id" => $id));
			$OBJECT_MAP["CardClass"][$id] = $obj;
		}
		return $obj;
	}
	...
	public static function create($clientDataObj, $subdomain=null) {
		...
		if (isset($clientDataObj->id) && self::exists(array("id" => $clientDataObj->id))) {
			return "A Card Class already exists with that id - you cannot have duplicate entries.";
		}
	...
	public static function createWhereArray($whereData, $prefix="") {
		...
		if (isset($whereData["id"])) {
			$whereArray["{$prefix}CardClass.id"] = $whereData["id"];
		}
		else {
			if (isset($whereData["name"])) {
				$whereArray["{$prefix}CardClass.name"] = $whereData["name"];
			}
			if (array_key_exists("figureSet", $whereData)) {
				if (isset($whereData["figureSet"]->id)) {
					$whereArray["{$prefix}CardClass.figureSetID"] = $whereData["figureSet"]->id;
				} else if ($whereData["figureSet"] == null) {
					$whereArray["{$prefix}CardClass.figureSetID"] = null;
				}
			}
		}
		
		if (isset($whereData["figureSet"])) {
			$whereArray = array_merge($whereArray, FigureSet::createWhereArray($whereData["figureSet"], "{$prefix}CardClass_figureSetID_"));
		}
	...
	// TODO: fill in this array with column(s) to order results by like this: array("CardClass.name" => "ASC")
		return array();
	}
	...
	protected function deleteLinks() {
		// N-1 Links
		if (Card::countEntries(array("Card.classID" => $this->id)) > 0) {
			return "Unable to delete Card Class because one or more Card is dependent on it.";
		}
