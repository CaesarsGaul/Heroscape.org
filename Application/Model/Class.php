...
class Class extends HS_DatabaseObject {
	protected $id; // Int
	...
	public static function fromDB($id, $subdomain = null) {
		global $OBJECT_MAP;
		if ( ! isset($OBJECT_MAP["Class"])) {
			$OBJECT_MAP["Class"] = array();
		}
		$obj = null;
		if (isset($OBJECT_MAP["Class"][$id])) {
			$obj = $OBJECT_MAP["Class"][$id];
		} else {
			$obj = parent::fromDBHelper(new self($subdomain), array("id" => $id));
			$OBJECT_MAP["Class"][$id] = $obj;
		}
		return $obj;
	}
	...
	public static function create($clientDataObj, $subdomain=null) {
		...
		if (isset($clientDataObj->id) && self::exists(array("id" => $clientDataObj->id))) {
			return "A Class already exists with that id - you cannot have duplicate entries.";
		}
	...
	public static function createWhereArray($whereData, $prefix="") {
		...
		if (isset($whereData["id"])) {
			$whereArray["{$prefix}Class.id"] = $whereData["id"];
		}
		else {
			if (isset($whereData["name"])) {
				$whereArray["{$prefix}Class.name"] = $whereData["name"];
			}
			if (array_key_exists("figureSet", $whereData)) {
				if (isset($whereData["figureSet"]->id)) {
					$whereArray["{$prefix}Class.figureSetID"] = $whereData["figureSet"]->id;
				} else if ($whereData["figureSet"] == null) {
					$whereArray["{$prefix}Class.figureSetID"] = null;
				}
			}
		}
		
		if (isset($whereData["figureSet"])) {
			$whereArray = array_merge($whereArray, FigureSet::createWhereArray($whereData["figureSet"], "{$prefix}Class_figureSetID_"));
		}
	...
	// TODO: fill in this array with column(s) to order results by like this: array("Class.name" => "ASC")
		return array();
	}
	...
	protected function deleteLinks() {
		...
		if (Card::countEntries(array("Card.classID" => $this->id)) > 0) {
			return "Unable to delete Class because one or more Card is dependent on it.";
		}
