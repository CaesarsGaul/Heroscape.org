...
class General extends HS_DatabaseObject {
	protected $id; // Int
	...
	public static function fromDB($id, $subdomain = null) {
		global $OBJECT_MAP;
		if ( ! isset($OBJECT_MAP["General"])) {
			$OBJECT_MAP["General"] = array();
		}
		$obj = null;
		if (isset($OBJECT_MAP["General"][$id])) {
			$obj = $OBJECT_MAP["General"][$id];
		} else {
			$obj = parent::fromDBHelper(new self($subdomain), array("id" => $id));
			$OBJECT_MAP["General"][$id] = $obj;
		}
		return $obj;
	}
	...
	public static function create($clientDataObj, $subdomain=null) {
		...
		if (isset($clientDataObj->id) && self::exists(array("id" => $clientDataObj->id))) {
			return "A General already exists with that id - you cannot have duplicate entries.";
		}
	...
	public static function createWhereArray($whereData, $prefix="") {
		...
		if (isset($whereData["id"])) {
			$whereArray["{$prefix}General.id"] = $whereData["id"];
		}
		else {
			if (isset($whereData["name"])) {
				$whereArray["{$prefix}General.name"] = $whereData["name"];
			}
			if (array_key_exists("figureSet", $whereData)) {
				if (isset($whereData["figureSet"]->id)) {
					$whereArray["{$prefix}General.figureSetID"] = $whereData["figureSet"]->id;
				} else if ($whereData["figureSet"] == null) {
					$whereArray["{$prefix}General.figureSetID"] = null;
				}
			}
		}
		
		if (isset($whereData["figureSet"])) {
			$whereArray = array_merge($whereArray, FigureSet::createWhereArray($whereData["figureSet"], "{$prefix}General_figureSetID_"));
		}
	...
	// TODO: fill in this array with column(s) to order results by like this: array("General.name" => "ASC")
		return array();
	}
	...
	protected function deleteLinks() {
		// N-1 Links
		if (Card::countEntries(array("Card.generalID" => $this->id)) > 0) {
			return "Unable to delete General because one or more Card is dependent on it.";
		}
