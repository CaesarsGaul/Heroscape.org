...
class HeroscapeCardPower extends HS_DatabaseObject {
	protected $id; // Int
	...
	public static function fromDB($id, $subdomain = null) {
		global $OBJECT_MAP;
		if ( ! isset($OBJECT_MAP["HeroscapeCardPower"])) {
			$OBJECT_MAP["HeroscapeCardPower"] = array();
		}
		$obj = null;
		if (isset($OBJECT_MAP["HeroscapeCardPower"][$id])) {
			$obj = $OBJECT_MAP["HeroscapeCardPower"][$id];
		} else {
			$obj = parent::fromDBHelper(new self($subdomain), array("id" => $id));
			$OBJECT_MAP["HeroscapeCardPower"][$id] = $obj;
		}
		return $obj;
	}
	...
	public static function createWhereArray($whereData, $prefix="") {
		...
		if (isset($whereData["id"])) {
			$whereArray["{$prefix}HeroscapeCardPower.id"] = $whereData["id"];
		}
		else {
			if (array_key_exists("card", $whereData)) {
				if (isset($whereData["card"]->id)) {
					$whereArray["{$prefix}HeroscapeCardPower.cardID"] = $whereData["card"]->id;
				} else if ($whereData["card"] == null) {
					$whereArray["{$prefix}HeroscapeCardPower.cardID"] = null;
				}
			}
			if (isset($whereData["name"])) {
				$whereArray["{$prefix}HeroscapeCardPower.name"] = $whereData["name"];
			}
			if (isset($whereData["description"])) {
				$whereArray["{$prefix}HeroscapeCardPower.description"] = $whereData["description"];
			}
		}
		
		if (isset($whereData["card"])) {
			$whereArray = array_merge($whereArray, HeroscapeCard::createWhereArray($whereData["card"], "{$prefix}HeroscapeCardPower_cardID_"));
		}
	...
	// TODO: fill in this array with column(s) to order results by like this: array("HeroscapeCardPower.name" => "ASC")
		return array("LENGTH(HeroscapeCardPower.order)" => "ASC", "HeroscapeCardPower.order" => "ASC");
	}
